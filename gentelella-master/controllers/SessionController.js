            ...
            if (result.length < 1) {
                ...
                bcrypt.compare(pass, result[0].passwd, function (err, res) {
                    ...
                    if (res == true) {
                        ...
                        var sessql = "SELECT users.User_ID, users.User_First, users.User_Last, users.email_address, users.Role, users.Group, users.ContactNo, users.username,users.isleader FROM capstone.users where binary username = ?"
                        var sesvalues = [user];
